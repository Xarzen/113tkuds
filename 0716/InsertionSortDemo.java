...
/*
 * 基本插入排序 Time Complexity: O(n^2)
 * 說明：外層迴圈執行n-1次，內層while迴圈在最壞的情況下需要比較和移動i次元素，
 * 總操作次數為1+2+...+(n-1) = O(n^2)。在最佳情況下如果陣列已排序則只需O(n)。
 */

/*
 * 二分插入排序 Time Complexity: O(n^2)
 * 說明：外層迴圈執行n-1次，二分搜尋找插入位置為O(log n)，但元素移動仍需O(n)，
 * 總時間複雜度為O(n×log n + n×n) = O(n^2)。
 */
